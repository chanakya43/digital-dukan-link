...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await loginVendor(username, password);
      toast.success("Login ho gaya!");
      navigate("/dashboard");
    } catch (err: any) {
      toast.error(err.message || "Galat username ya password");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 bg-primary px-4 py-3 shadow-md">
        <div className="container flex items-center gap-3">
          ...
          <h1 className="font-heading font-bold text-lg text-primary-foreground">Vyapari Login</h1>
        </div>
      </header>
      ...
      <div className="px-4 py-10">
        <div className="container max-w-sm mx-auto">
          <div className="text-center mb-8 opacity-0 animate-fade-up">
            ...
            <h2 className="font-heading font-bold text-xl text-foreground">Apni Dukan Manage Karein</h2>
            <p className="text-sm text-muted-foreground mt-1">Apna username aur password daalo</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4 opacity-0 animate-fade-up" style={{ animationDelay: "100ms" }}>
            <div>
              ...
              <input type="text" required maxLength={30} placeholder="Apna username"
            ...
            <div>
              ...
              <input type="password" required placeholder="Apna password"
            ...
            <p className="text-center text-xs text-muted-foreground">
              Naya Vyapari ho?{" "}
              <Link to="/create-shop" className="text-primary font-semibold hover:underline">Dukan Banayein</Link>
            </p>
          </form>
        </div>
      </div>
